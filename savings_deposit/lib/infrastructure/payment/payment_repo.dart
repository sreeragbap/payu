...
class PaymentRepo implements IPaymentRepo {
  ...
  //   try {
  //     String basicAuth =
  //         'Basic ${base64.encode(utf8.encode('${PayuPaymentKeys.keyId}:${PayuPaymentKeys.keySecret}'))}';
  ...
  Future<Either<PaymentFailure, String>> logPaymentDetails(
    ...
    try {
      ...
      return const Left(PaymentFailure.clientFailure());
    }
  }
  ...
  @override
  Future<Either<PaymentFailure, List<TransactionCharge>>>
      getPaymentTransactionCharges() async {
    try {
      var parameters = {"Type": "TransChargerequest", "Ver": 1.0, "Data": {}};
      var requestJson = jsonEncode(parameters);
      Map<String, dynamic> queryParameter = {"RequestJson": requestJson};
      Uri uri = Uri.http("docker.mactech.net.in:8019",
          "/PayUTransactionCharges", queryParameter);
      Response response = await http.Client().get(uri);
      List<TransactionCharge> transactionCharges = [];
      if (response.statusCode == 200) {
        var models = jsonDecode(response.body);
        for (var model in models) {
          TransactionCharge transactionCharge =
              TransactionCharge.fromJson(model);
          transactionCharges.add(transactionCharge);
        }
        return Right(transactionCharges);
      } else {
        return const Left(PaymentFailure.serverFailure());
      }
    } catch (e) {
